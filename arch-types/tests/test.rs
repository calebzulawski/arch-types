...
mod x86 {
    fn sse(tag: arch_types::requires_features! { "sse" }) {
        assert!(arch_types::has_features!(tag => "sse"));
    }

    fn sse2(tag: arch_types::requires_features! { "sse2" }) {
        assert!(arch_types::has_features!(tag => "sse2"));
    }

    fn sse_avx(tag: arch_types::requires_features! { "sse", "avx" }) {
        assert!(arch_types::has_features!(tag => "sse", "avx"));
    }

    fn avx2(tag: arch_types::requires_features! { "avx2" }) {
